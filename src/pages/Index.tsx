...
const Index = () => {
  ...
  const showTypingIndicator =
    isLoading && !showSearchProgress && messages[messages.length - 1]?.role !== "assistant";
          ...
          >
            <div className="space-y-4">
              ...
              <p className="mx-auto max-w-md text-sm text-muted-foreground/80">
                Tell Prima what you need — we search 50+ retailers, rank by value, delivery & reviews,
                and handle checkout across every store.
              </p>
            </div>

            {/* How it works */}
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-6 mx-auto max-w-lg">
              {[
            ...
            <Button
              ...
              className="rounded-xl px-8 font-display text-base min-h-[48px]"
            >
            ...
            <div className="space-y-3">
              <p className="text-sm font-medium text-muted-foreground">Or try a scenario:</p>
            ...
            {sessions.length > 0 && (
              <div className="pt-4">
                <p className="text-sm text-muted-foreground">
                  You have {sessions.length} recent session{sessions.length > 1 ? "s" : ""}
                </p>
                <Button
                  variant="link"
                  onClick={() => {
  ...
  return (
    <div className="flex h-dvh bg-background">
      ...
      <div className="flex flex-1 flex-col">
        {/* Header */}
        <header className="flex items-center gap-3 border-b border-border px-4 py-3 md:px-6 safe-top">
          ...
          <div className="mx-auto max-w-2xl space-y-4">
            ...
            {messages.map((msg, i) => (
              <ChatMessageBubble
                ...
                onReplaceItem={(name) => sendMessage(`Replace "${name}" with an alternative`)}
        ...
        {/* Input */}
        <div className="border-t border-border p-3 md:p-4 safe-bottom">
